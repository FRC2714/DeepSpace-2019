...
public class ADIS16470_IMU extends GyroBase implements Gyro, PIDSource, Sendable {
    ...
    public ADIS16470_IMU(Axis yaw_axis, SPI.Port port) {
        ...
        int id_thing = readRegister(kRegPROD_ID);
        System.out.println("id_thing: " + id_thing);
    ...
    private void acquire() {
        ...
        while (!m_freed.get()) {
            ...
            data_count = m_spi.readAutoReceivedData(readBuf, 0, 0); // Read number of bytes currently stored in the
                                                                    // buffer
            array_offset = data_count % 92; // Look for "extra" data This is 92 not 23 like in C++ b/c everything is
            ...
            for (int i = 0; i < data_to_read; i += 92) { // Process each set of 23 bytes (timestamp + 23 data) * 4
                                                         // (32-bit ints)
                ...
                // Calculate checksum
                int calc_checksum = 0;
                for (int k = 2; k < 20; k++) { // Cycle through STATUS, XYZ GYRO, XYZ ACCEL, TEMP, COUNTER (Ignore DUT
                                               // Checksum)
